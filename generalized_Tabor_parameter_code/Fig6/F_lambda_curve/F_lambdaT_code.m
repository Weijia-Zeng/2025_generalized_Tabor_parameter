...
start = -3;
cutoff = 3;
lambda_T = logspace(start,cutoff,(cutoff-start)*10+1);
...
for i = 1:length(lambda)
    ...
    for j=1:length(H)
        ...
        for k=1:n-1
            F(j,i) = F(j,i)+16/3*pi*lambda_E^2*Rmesh(k)*dR*((H(j)+Rmesh(k)^2/2-w(k)+1)^(-3)-(H(j)+Rmesh(k)^2/2-w(k)+1)^(-9));
        end
        F(j,i) = F(j,i)+16/3*pi*lambda_E^2*Rf*dR*((H(j)+Rf^2/2-wn+1)^(-3)-(H(j)+Rf^2/2-wn+1)^(-9));
...
title(['$\bar{R_f}=$',num2str(Rf),'$\quad \bar{R_s}=$',num2str(Rs),'$\quad \lambda_E=$',num2str(lambda_E),'$\quad \lambda_p=\infty$'],'Interpreter','latex','fontsize',12)
...
plot(lambda_T,Fmax/pi./lambda_E.^2,'linewidth',2,'Color','#E49273')
