...
start = -5;
cutoff = 1000;
H = linspace(start,cutoff,(cutoff-start)*1+1);
v = 0.165;
...
lambda = -lambda_E^2/2./lambda_T+sqrt( (lambda_E^2/2./lambda_T).^2+lambda_E^2 );
...
for i=1:length(lambda)
    ...
    for j=length(H):-1:1
        ...
        while(norm(R)>1e-8 && iteration ~= iteration_max)
            ...
            phin = ( 4*phi(n-1)-phi(n-2)+2*(1-v)*dR*T(i) ) / ( 3-2*v*dR/Rf );

            for k=2:(n-2) 
                f(k) = phi(k)/Rmesh(k) * (w(k+1)-2*w(k)+w(k-1))/dR^2 + 1/Rmesh(k) * (phi(k+1)-phi(k-1))/(2*dR) * (w(k+1)-w(k-1))/(2*dR) + 8*lambda_E^2/3*((H(j)+1+Rmesh(k)^2/2-w(k))^(-3)-(H(j)+1+Rmesh(k)^2/2-w(k))^(-9));
                ...
                e(k,k) = phi(k)/Rmesh(k) * (-2)/dR^2 + 8*lambda_E^2/3*(3*(H(j)+1+Rmesh(k)^2/2-w(k))^(-4)-9*(H(j)+1+Rmesh(k)^2/2-w(k))^(-10));
                e(k,k+1) = phi(k)/Rmesh(k) * 1/dR^2 + 1/Rmesh(k) * (phi(k+1)-phi(k-1))/(2*dR) * 1/(2*dR);
            ...
            f(1) = phi(1)/Rmesh(1) * (w(2)-2*w(1)+w0)/dR^2 + 1/Rmesh(1) * (phi(2)-phi0)/(2*dR) * (w(2)-w0)/(2*dR) + 8*lambda_E^2/3*((H(j)+1+Rmesh(1)^2/2-w(1))^(-3)-(H(j)+1+Rmesh(1)^2/2-w(1))^(-9));
            g(1) = (phi(2)-phi0)/(2*dR) - phi(1)/Rmesh(1) + Rmesh(1) * (phi(2)-2*phi(1)+phi0)/dR^2 + 1/2 * ( (w(2)-w0)/(2*dR) )^2;

            e(1,1) =  phi(1)/Rmesh(1) * (-2+4/3)/dR^2 + 1/Rmesh(1) * (phi(2)-phi0)/(2*dR) * (-4/3)/(2*dR) + 8*lambda_E^2/3*(3*(H(j)+1+Rmesh(1)^2/2-w(1))^(-4)-9*(H(j)+1+Rmesh(1)^2/2-w(1))^(-10));
            ...
            f(n-1) = phi(n-1)/Rmesh(n-1) * (wn-2*w(n-1)+w(n-2))/dR^2 + 1/Rmesh(n-1) * (phin-phi(n-2))/(2*dR) * (wn-w(n-2))/(2*dR) + 8*lambda_E^2/3*((H(j)+1+Rmesh(n-1)^2/2-w(n-1))^(-3)-(H(j)+1+Rmesh(n-1)^2/2-w(n-1))^(-9));
            ...
            e(n-1,n-1) = phi(n-1)/Rmesh(n-1) * (-2)/dR^2 + 8*lambda_E^2/3*(3*(H(j)+1+Rmesh(n-1)^2/2-w(n-1))^(-4)-9*(H(j)+1+Rmesh(n-1)^2/2-w(n-1))^(-10));
        ...
        for k=1:n-1
            F_1(j,i) = F_1(j,i)+16/3*pi*lambda_E^2*Rmesh(k)*dR*((H(j)+Rmesh(k)^2/2-w(k)+1)^(-3)-(H(j)+Rmesh(k)^2/2-w(k)+1)^(-9));
        end
        F_1(j,i) = F_1(j,i)+16/3*pi*lambda_E^2*Rf*dR*((H(j)+Rf^2/2-wn+1)^(-3)-(H(j)+Rf^2/2-wn+1)^(-9));
...
for i=1:length(lambda)
    ...
    for j=1:length(H)
        ...
        for k=1:n-1
            F_2(j,i) = F_2(j,i)+16/3*pi*lambda_E^2*Rmesh(k)*dR*((H(j)+Rmesh(k)^2/2-w(k)+1)^(-3)-(H(j)+Rmesh(k)^2/2-w(k)+1)^(-9));
        end
        F_2(j,i) = F_2(j,i)+16/3*pi*lambda_E^2*Rf*dR*((H(j)+Rf^2/2-wn+1)^(-3)-(H(j)+Rf^2/2-wn+1)^(-9));
...
figure(1)
hold on
leg = {'$Bradley\ limit$','$\lambda_T=10^{-2}$','$\lambda_T=10^{-1}$','$\lambda_T=10^{0}$'};
c = [.01 .72 .77
    .99 .49 .00
    .68 .42 .89];
...
for i=1:length(lambda)
    if one_line(i) == 1
        plot(H,F_1(:,i)/(pi*lambda_E^2),'color',c(i,:),'LineWidth',2)
    else
        plot(H(Index(1,i)+1:end),F_1(Index(1,i)+1:end,i)/(pi*lambda_E^2),'color',c(i,:),'linewidth',2)
    end
end
...
for i=1:length(lambda)
    if one_line(i) == 1
        ...
        plot(H(1:Index(2,i)),F_2(1:Index(2,i),i)/pi/lambda_E^2,'color',c(i,:),'linewidth',2)
    end
end
...
for i=1:length(lambda)
    if one_line(i) == 1
        ...
        line([H(Index(1,i)),H(Index(1,i)+1)],[F_2(Index(1,i),i)/pi/lambda_E^2,F_1(Index(1,i)+1,i)/pi/lambda_E^2],'color',c(i,:),'linestyle','--')
        line([H(Index(2,i)),H(Index(2,i)+1)],[F_2(Index(2,i),i)/pi/lambda_E^2,F_1(Index(2,i)+1,i)/pi/lambda_E^2],'color',c(i,:),'linestyle','--')
    end
end
